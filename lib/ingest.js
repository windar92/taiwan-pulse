...
async function handleWater(feed) {
  ...
  for (const rec of (Array.isArray(arr) ? arr : [])) {
    ...
    const loc = locate(text, null);
    if (!loc) continue;
    // 嘗試組標題：影響行政區 + 停水原因；抓不到就用通用字樣
    const pick = (...keys) => { for (const k of Object.keys(rec)) if (keys.some((s) => k.includes(s))) { const v = rec[k]; if (v) return String(v); } return ""; };
    const area = pick("影響行政區", "停水地區", "影響", "區域") || loc.place;
    const reason = pick("停水原因", "原因", "說明");
    const when = pick("停水", "案件日期", "日期", "時間");
    const title = `停水通知：${String(area).slice(0, 40)}`;
    const summary = [reason, when].filter(Boolean).join("；").slice(0, 600) || "台水公司停水資訊，詳見原文。";
    const hash = createHash("sha1").update(`${feed.id}|${pick("案件編號", "編號", "id") || blob.slice(0, 80)}`).digest("hex");
